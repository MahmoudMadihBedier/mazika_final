...
class _MoodBody extends StatelessWidget {
  ...
  Color get _color {
    switch (context?.inferredMood) {
      ...
      default: return const Color(0xFF4CC9F0);
    }
  }
  ...
  Widget build(BuildContext ctx) {
    return SafeArea(child: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Mood', style: GoogleFonts.syne(fontSize: 28, fontWeight: FontWeight.w800, color: Colors.white)),
        Text('What the AI is sensing', style: GoogleFonts.dmSans(fontSize: 13, color: Colors.white.withOpacity(0.4))),
        const SizedBox(height: 28),
        Center(child: _MoodOrb(color: _color, label: context?.moodLabel ?? 'Unknown')),
        const SizedBox(height: 28),
        _label('Why this mood'),
        const SizedBox(height: 10),
        _reason(Icons.place_outlined, 'You\'re ${context?.activityLabel.toLowerCase() ?? 'relaxing'}'),
        const SizedBox(height: 8),
        _reason(Icons.access_time_rounded, _timeReason()),
        const SizedBox(height: 8),
        _reason(Icons.speed_rounded, 'Speed: ${context?.gpsSpeed.toStringAsFixed(1) ?? '0.0'} m/s'),
  ...
  Widget _label(String t) => Text(t.toUpperCase(), style: GoogleFonts.dmSans(fontSize: 11, letterSpacing: 1.5, fontWeight: FontWeight.w600, color: Colors.white.withOpacity(0.35)));
  ...
  Widget _reason(IconData icon, String text) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(text, style: GoogleFonts.dmSans(color: Colors.white.withOpacity(0.7), fontSize: 14))),
    ]),
  );
}
...
class _MoodOrb extends StatefulWidget {
  ...
  @override State<_MoodOrb> createState() => _MoodOrbState();
}

class _MoodOrbState extends State<_MoodOrb> with SingleTickerProviderStateMixin {
      ...
      return Transform.translate(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: RadialGradient(colors: [widget.color, widget.color.withOpacity(0.5)]),
            boxShadow: [BoxShadow(color: widget.color.withOpacity(0.4 + _anim.value * 0.2), blurRadius: 40 + _anim.value * 20, spreadRadius: 5)],
          ),
          alignment: Alignment.center,
          child: Text(widget.label, style: GoogleFonts.syne(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
        ),
      );
    });
  }
...
class _MoodChips extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Wrap(spacing: 10, runSpacing: 10, children: _moods.map(((label, mood, color)) {
      return GestureDetector(
        onTap: () => context.read<MoodBloc>().add(MoodOverrideRequested(mood)),
        child: Container(
          ...
          child: Text(label, style: GoogleFonts.dmSans(color: color, fontWeight: FontWeight.w600, fontSize: 14)),
        ),
      );
    }).toList());
  }
...
class _ScoreBreakdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: factors.map((f) {
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 8),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(f.$1, style: GoogleFonts.dmSans(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
              Text('${(f.$2 * 100).toInt()}%', style: GoogleFonts.dmSans(color: moodColor, fontSize: 13, fontWeight: FontWeight.w700)),
            ]),
            const SizedBox(height: 4),
            LinearProgressIndicator(value: f.$2,
            ...
            Text(f.$3, style: GoogleFonts.dmSans(color: Colors.white.withOpacity(0.35), fontSize: 11)),
          ]),
        );
      }).toList()),
    );
